...
interface ChangeCanvasPasswordModalProps {
  canvas: CanvasSummary | null;
  isOpen: boolean;
  onClose: () => void;
}

export const ChangeCanvasPasswordModal: React.FC<ChangeCanvasPasswordModalProps> = ({
  canvas,
  isOpen,
  onClose,
}) => {
  ...
  const { changeCanvasPassword } = useCanvasStore();

  useFocusTrap(modalRef, isOpen && canvas !== null, onClose, oldPasswordInputRef);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await changeCanvasPassword(canvas.id, oldPassword, newPassword);
      onClose();
    } catch (err: any) {
            ...
            <div className="flex flex-col select-none pr-8">
              <div className="flex items-center gap-2.5 mb-1.5">
                ...
                <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
                  Change Password<span className="text-[#DE6B80]">.</span>
                </h2>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                Update master password for &ldquo;{canvas.name}&rdquo;
              </p>
            </div>
...
export default ChangeCanvasPasswordModal;
